...
| File | Job |
|---|---|
| `src/config.py` | All settings + the 6 trigger definitions |
| `src/ghl_client.py` | GoHighLevel API — contacts, tags, notes, workflows |
...
| `src/store.py` | SQLite persistence for queue, attempts, bookings, dashboard stats |
| `dashboard/index.html` | Live ops dashboard (auto-refreshes every 30s) |

## Quick start

```bash
cd ~/Projects/vapi-warm-caller
cp .env.example .env          # fill in GHL, Vapi, GCal keys
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt

# Verify the full loop works without credentials:
python scripts/demo_loop.py

# Once .env is filled in:
python scripts/validate_setup.py
python scripts/create_assistant.py
gunicorn -w 2 -b 0.0.0.0:8080 src.webhook_server:app   # needs public HTTPS
python -m src.orchestrator --once --dry-run            # poll GHL, no dial
python -m src.orchestrator --once                        # live test dial
```

See [DEPLOYMENT.md](DEPLOYMENT.md) for the full 45-minute setup guide from Claude.
